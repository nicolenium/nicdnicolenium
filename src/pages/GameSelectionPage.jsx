...
const TIERS = ['All', 'Tier 1 (Most Popular)', 'Tier 2 (Popular)', 'Tier 3 (Moderate)', 'Tier 4 (Educational)'];
...
export default function GameSelectionPage() {
  ...
  const handleStartGame = (config) => {
    ...
    if (selectedGame.path) {
      navigate(selectedGame.path, { state: { gameConfig: config } });
    }
  };

  return (
    <div className="min-h-screen flex flex-col bg-background">
      <Helmet><title>Game Library | NICOLENIUM</title></Helmet>
      <Header />
        ...
        <div className="max-w-[1400px] mx-auto space-y-10">
          ...
          <div className="text-center space-y-4 pt-6">
            <h1 className="text-5xl md:text-7xl font-black text-foreground uppercase tracking-tight">
              Complete <span className="text-primary">Library</span>
            </h1>
            <p className="text-muted-foreground text-xl max-w-2xl mx-auto font-medium">
              Explore 33 premium educational experiences spanning strategy, puzzles, and trivia.
            </p>
          </div>
