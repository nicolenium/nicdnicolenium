...
const CarromPage = () => {
  ...
  useEffect(() => {
    const inviteId = localStorage.getItem('nicd_carrom_invite_match');
    if (inviteId) {
      setGameMode('human_vs_human');
      localStorage.removeItem('nicd_carrom_invite_match');
    }
  }, []);
  ...
  return (
    <>
      <Helmet><title>NICD Carrom | Nicolenium Premium</title></Helmet>
      <FullScreenGameLayout title="NICD Premium Carrom" mode="Classic" onExit={() => setGameMode(null)} rightPanelContent={rightPanel}>
          ...
          {!gameMode ? (
            <div className="w-full max-w-4xl flex flex-col items-center z-10">
              <div className="w-full flex justify-end gap-3 mb-6 px-4">
                <InviteButton gameType="carrom" />
                <RequestButton gameType="carrom" />
              </div>
              <GameModeSelector onStart={(mode, diff) => { 
                setGameMode(typeof mode === 'string' ? mode : 'human_vs_ai'); 
                setDifficulty(typeof diff === 'string' ? diff : 'medium'); 
              }} />
            </div>
          ) : (
            <div className="text-center p-12 bg-card/80 backdrop-blur-md border border-white/10 rounded-3xl shadow-2xl max-w-lg w-full mx-auto z-10">
              <BrandedGameHeader gameTitle="NICD Premium Carrom" />
              <p className="text-muted-foreground mb-8 font-medium mt-4">
                Mode: {String(gameMode)} | Difficulty: {difficulty ? String(difficulty) : 'N/A'}
              </p>
